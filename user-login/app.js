...
app.post('/addTask', async (req, res) => {
    try {
        ...
        await pool.query(query, [userId, title, description, priority, due_date]);
...
app.delete('/deletePlanner/:id', async (req, res) => {
    try {
        ...
        const query = 'DELETE FROM planners WHERE id = ? AND user_id = ?';
        const [result] = await pool.query(query, [plannerId, userId]);

        if (result.affectedRows === 0) {
            return res.status(404).send('Planner not found or not authorized to delete this planner');
        }

        res.send('Planner deleted successfully');
    } catch (error) {
